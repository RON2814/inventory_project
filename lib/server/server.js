...
app.get("/get-product", async (req, res) => {
    try {
        ...
        let product = await collection.find().limit(limit).skip(limit * (page - 1)).sort({ $natural: -1 }).toArray();
...
app.get("/search-product", async (req, res) => {
    try {
        ...
        let collection = await db.collection(col_products);
        //let result = await collection.find({ $text: { $search: searchQuery } },
...
app.post("/add-product", async (req, res) => { // Fixed path
    try {
        ...
        res.status(500).json({ error: "Can't add Product. Error is occur", detailed: e.toString() });
        console.log(e);
    }
});

...
const colProducts = "products",
  colAccount = "accounts",
  colArchive = "archive_products"; // <- Collection name
...
app.get("/get-product", async (req, res) => {
  try {
    ...
    res
      .status(500)
      .json({
        error: "Products could not be returned.",
        detailed: e.toString(),
      });
    console.log(e);
  }
...
app.post("/insert-product", async (req, res) => {
  ...
  try {
    ...
    res
      .status(200)
      .json({ request: "Insert success " + result, isInserted: "true" });
  } catch (e) {
    res
      .status(500)
      .json({
        error: "Can't Insert Product. Error is occur",
        detailed: e.toString(),
      });
    console.log(e);
  }
...
app.get("/get-total-product", async (req, res) => {
  try {
    ...
    res
      .status(500)
      .json({ error: "Can't fetch total product", detailed: e.toString() });
    console.log(e);
  }
});
...
app.post("/update-product", async (req, res) => {
  try {
    ...
    if (product && product.history && product.history.length > 0) {
      console.log("History Array: ", product.history);

      const historyIds = product.history.map((h) => h.id);
      console.log("History IDs: ", historyIds);

      maxId = Math.max(...historyIds);
      ...
      let result = await collection.updateOne(
        ...
        },
        {
          ...
          $push: {
            history: {
              id: newHistoryId,
              is_added: upProd.is_added,
              qty: upProd.edited_qty,
              updated_at: upProd.updated_at,
            },
          },
        },
        { returnOriginal: false }
    ...
    res
      .status(500)
      .json({
        error: "Can't Update Product. Error is occur",
  ...
  try {
    ...
    try {
      ...
      if (!ObjectId.isValid(prodId)) {
        ...
        return res
          .status(400)
          .json({ message: "Invalid Product ID", results: responses });
      }
      ...
      if (!result) {
        ...
        return res
          .status(404)
          .json({ message: "Product not found", results: responses });
      }
      ...
      responses.push({
        ...
        error: true,
        detailed: innerError.toString(),
      });
      return res
        .status(500)
        .json({ message: "Delete operation failed", results: responses });
    }
    ...
    res
      .status(500)
      .json({
        error: "Can't delete product. Error occurred.",
        detailed: e.toString(),
      });
    console.log(e);
  }
...
app.get("/fetch-history", async (req, res) => {
  try {
      ...
      .aggregate([
        ...
        },
        {
          $project: {
            ...
            is_added: "$history.is_added",
            qty: "$history.qty",
            updated_at: "$history.updated_at",
          },
        },
        {
          $sort: { "history.updated_at": -1 }, // Sort by updated_at in descending order
        },
        {
          $skip: limit * (page - 1),
        },
    ...
    res
      .status(500)
      .json({ error: "Fetching history..", detailed: e.toString() });
    console.log(e);
  }
});
...
app.post("/login", async (req, res) => {
  try {
    ...
    if (!check) {
      res
        .status(401)
        .json({ request: "username connot found.", access: false });
    } else {
      ...
      if (isPwdMatch) {
        ...
        res
          .status(401)
          .json({ request: "Password is incorrect", access: false });
      }
    }
  } catch (e) {
...
app.post("/add-account", async (req, res) => {
  try {
    ...
    if (existingUser) {
      ...
      res
        .status(200)
        .json({ request: "Insert success username: " + account.username });
    }
  } catch (e) {
    res
      .status(500)
      .json({ error: "Can't add Account. Error is occur", detailed: e });
    console.log(e);
  }
});
